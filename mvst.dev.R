robust.mvst.dev <- function(x, params, robAlpha, robFuncType, OmegaInv=NULL){
  ...
  robFunc = get.robust.func( k=qf(1-robAlpha, df1=p, df2=nu), robFuncType )
    
    if(nu>100000){ #Approximately multivariate skew-normal, use that density
    if(p==1)
      return( -log(2) + p/2*log(2*pi) + 1/2*log(detOmega) +
          robFunc( 1/2*y^2 - log( pnorm( alpha/omega*(x-xi) ) ) ) )
    return( -log(2) + p/2*log(2*pi) + 1/2*log(detOmega) +
        robFunc( 1/2*t(y)%*%y - log( pnorm( (t(alpha)/omega)%*%matrix(x-xi) ) ) ) )
  } else {
    Q = t(y)%*%y
    if(p==1)
      return( -log(2) + p/2*log(2*pi) + 1/2*log(detOmega) +
